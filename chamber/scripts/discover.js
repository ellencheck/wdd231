document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!last) {
    ...
    const diffDays = Math.floor((now - Number(last)) / (1000 * 60 * 60 * 24));
    if (diffDays < 1) {
  ...
  if (lastModifiedSpan) {
    let lastModifiedRaw = document.lastModified;
    if (!lastModifiedRaw || lastModifiedRaw === "0") {
      // Если lastModified пустое, используем текущую дату
      lastModifiedRaw = new Date().toISOString();
    }
